...
export class PdfJsViewerComponent implements OnInit, OnDestroy {
  ...
  static lastID = 0;
  @Input()
  public viewerId = `ngx-pdfjs-id${++PdfJsViewerComponent.lastID}`;
  ...
  public get page(): number {
    ...
    if (this.diagnosticLogs && !this.PDFViewerApplication) {
      console.warn(
        'Document is not loaded yet. Retrieve page number after full load.'
      );
    }
    ...
    document.addEventListener('webviewerloaded', () => {
      ...
      this.PDFViewerApplication.initializedPromise.then(() => {
        ...
        eventBus.on('pagesinit', () => {
          if (this.diagnosticLogs)
            console.debug('PdfJsViewer: All pages have been rendered!');
        });
        ...
        // Page change.
        eventBus.on('pagechanging', (event: any) => {
          if (this.diagnosticLogs)
            console.debug(
        ...
        // Scale change.
        eventBus.on('scalechanging', (event: any) => {
  ...
  private configureVisibleFeatures() {
    ...
    toolbarElement.querySelector(
      '.horizontalToolbarSeparator:first-of-type'
    ).hidden = !this.openFile;
